...
def select_patch(img, img_out, pos_y, pos_x, patch_height, patch_width,
    ...
    for y in range(overlap_height, img_height - patch_height):
        for x in range(overlap_width, img_width - patch_width):
            err_map[y, x] = 0
            # Overlap error
            overlap_multiplier = 0.5 * alpha if pos_y > 0 and pos_x > 0 else alpha
            if itr > 0:
                overlap_multiplier *= 0.5
            if pos_y > 0:
                err_map[y, x] += overlap_multiplier * ssd(img[y - overlap_height:y, x:x + ioa_width], img_out_above)
            if pos_x > 0:
                err_map[y, x] += overlap_multiplier * ssd(img[y:y + iol_height, x - overlap_width:x], img_out_left)
            # Target error
